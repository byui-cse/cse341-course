$(() => {
  ...
  $('table.rubric tbody td:not(:first-child):not(:last-child)').click(
    function () {
      // Show selection
  ...
  function calcGrade() {
    ...
    const selectedCount = $(
      'table.rubric tbody td:not(:first-child):not(:last-child).selected'
    ).length;
    ...
    $rows.each(function (i) {
      ...
      const score =
        +$(`table.rubric thead tr:last-child :nth-child(${colNum})`)
          .text()
          .replace(/%/g, '') / 100;
      grade += score * weight;
    });
